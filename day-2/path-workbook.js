(()=>{
  ...
  const researchText=()=>[["research-workplace","Workplace research"],["research-occupation","Occupation title and region"],["research-duties","Occupation duties"],["research-requirements","Occupation requirements"],["research-source-one","Source 1 and supported claim"],["research-source-two","Source 2 and supported claim"],["research-unknowns","Unknowns and who to ask"],['research-fact','Research check: a fact'],['research-assumption','Research check: an assumption'],['research-question','Research check: a question']].map(([id,label])=>label+'\n'+(document.getElementById(id).value.trim()||'[not answered]')).join('\n\n');
  ...
  window.addEventListener('beforeunload',event=>{if(document.querySelector('#learn textarea') && ([...document.querySelectorAll('#learn textarea')].some(field=>field.value.trim()) || Object.values(states).some(s=>Object.values(s.answers).some(v=>v.trim())))){event.preventDefault();event.returnValue='';}});
})();
